"""Mapping between local actors and authenticated users (user core domain)."""
...
class UserActor(SQLModel, table=True):
    """Tracks which authenticated user claimed a local actor."""

    __tablename__ = "user_actors"

    actor_id: str = Field(primary_key=True, max_length=255, description="Local actor id")
    user_id: UUID = Field(foreign_key="users.id", description="Authenticated user id")
